...
from network_flow import MinCostFlowTracker
...
for i in range(1, track_len):
    images.append(cv2.imread('E:\\PyProjects\\datasets\\MOT17\\train\\MOT17-10-SDP\\img1\\00%04d.jpg' % i))

gt = np.genfromtxt('E:\\PyProjects\\datasets\\MOT17\\train\\MOT17-10-SDP\\gt\\gt.txt', delimiter=',')
gt = gt[(gt[:, 0] < track_len) & (gt[:, 6] == 1), :]
mask = (gt[:, 7] == 1) | (gt[:, 7] == 2) | (gt[:, 7] == 7)
gt = gt[mask].astype(np.int32)

dets = np.genfromtxt('E:\\PyProjects\\datasets\\MOT17\\train\\MOT17-10-SDP\\det\\det.txt', delimiter=',')
...
def get_tracks(entry_exit_cost=None, thresh=None,
    ...
    track_model = MinCostFlowTracker(observation_model=observation_model,
                                     ...
                                     entry_exit_cost=entry_exit_cost, min_flow=1,
...
cv2.namedWindow('{}'.format("1"), cv2.WINDOW_NORMAL)
for i in range(track_len - 1):
    boxes = track[track[:, 0] == i + 1]
    id_list = list(boxes[:, 1])
    boxes = boxes[:, 2:6]
    for bbox, id in zip(boxes, id_list):
        x_tl = (bbox[0], bbox[1])
        x_br = (bbox[0] + bbox[2], bbox[1] + bbox[3])
        cv2.rectangle(images[i], x_tl, x_br, colors(id), 2)
        cv2.putText(images[i], "{}".format(id), (int(bbox[0]), int(bbox[1])), 0,
                    5e-3 * 200, colors(id), 2)
    cv2.imshow('{}'.format(1), images[i])
