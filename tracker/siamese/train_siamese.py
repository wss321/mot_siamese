    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train']:
            ...
            for data in dataloaders[phase]:
                ...
                # forward
                cla, pscore = model(inputs, pos)
                _, nscore = model(inputs, neg)
                ...
                _, preds = torch.max(cla.data, 1)
                _, p_preds = torch.max(pscore.data, 1)
                _, n_preds = torch.max(nscore.data, 1)
                loss_id = criterion1(cla, labels)
                loss_verif = (criterion2(pscore, labels_1) + criterion2(nscore, labels_0)) * 0.5 * opt.alpha
...
optimizer_ft = optim.SGD([
    ...
    {'params': model.fc_cl.parameters(), 'lr': opt.lr},
    {'params': model.id_classifier.parameters(), 'lr': opt.lr},
    {'params': model.fc_verif.parameters(), 'lr': opt.lr},
    {'params': model.bce.parameters(), 'lr': opt.lr}
